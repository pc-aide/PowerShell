...
$Form.Text = "Add AD Users to AD Groups"
...
if ($Form.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    foreach ($Group in $Groups) {
        ...
        foreach ($User in $Users) {
            ...
            if (Get-ADGroupMember -Identity $Group | Where-Object { $_.SamAccountName -eq $User }) {
                ...
                try {
                    Add-ADGroupMember -Identity $Group -Members $User -Server $PDC -ErrorAction Stop
                    $LogEntry = [PSCustomObject]@{
                        Users = $User
                        Groups = $Group
                        Error = ""
    ...
    $Log | Export-Csv -Path "C:\temp\Add_users_to_mul_groups.log" -NoTypeInformation -Encoding UTF8 -Delimiter ";" -Force -Append

    # Afficher en rouge les erreurs et en vert les ajouts dans la console PowerShell
