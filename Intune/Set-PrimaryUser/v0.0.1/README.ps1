...
$User_resource = "users"
    ...
    try {
        ...
        # If empty or null, return all users
...
foreach($PrimaryUser in $List_PrimaryUser){

    if(!$PrimaryUser.DeviceName){
    Write-Host
    write-host "Intune Device Name:" -f Yellow
    $PrimaryUser.DeviceName = Read-Host
    ...
    if(!$PrimaryUser.UPN){
        ...
        $PrimaryUser.UPN = Read-Host

    }

    $Device = Get-Win10IntuneManagedDevice -deviceName $PrimaryUser.DeviceName
    ...
    if($Device){
        ...
        $User = Get-AADUser -userPrincipalName $PrimaryUser.UPN
        ...
        $AADUserName = $User.displayName
            ...
            if($IntuneDevicePrimaryUser -notmatch $User.id){
                ...
                if($SetIntuneDevicePrimaryUser -eq ""){
                    ...
                    Write-Host "User"$User.displayName"set as Primary User for device" $PrimaryUser.DeviceName"..." -ForegroundColor Green
    ...
    # Error : if Intune Device not exist (validate) :
    else {
        Write-Host "Intune Device" $PrimaryUser.DeviceName" can't be found..." -ForegroundColor Red
    }
